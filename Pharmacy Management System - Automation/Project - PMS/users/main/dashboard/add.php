...
if ( !$connection ) {
    ...
    if ( 'addManager' == $action ) {
        ...
        if ( $fname && $lname && $lname && $phone && $password ) {
            $query = "INSERT INTO managers(fname,lname,email,phone,password) VALUES ('{$fname}','$lname','$email','$phone','$password')";
        ...
        if ( $fname && $lname && $lname && $phone && $password ) {
            $query = "INSERT INTO pharmacists(fname,lname,email,phone,password) VALUES ('{$fname}','$lname','$email','$phone','$password')";
            mysqli_query( $connection, $query );
        ...
        if ( $fname && $lname && $lname && $phone && $password ) {
            $query = "INSERT INTO salesmans(fname,lname,email,phone,password) VALUES ('{$fname}','$lname','$email','$phone','$password')";
            mysqli_query( $connection, $query );
        ...
        if ( $fname && $lname && $email && $phone && $oldPassword && $newPassword ) {
            ...
            if ( $data = mysqli_fetch_assoc( $result ) ) {
                ...
                if ( password_verify( $oldPassword, $_password ) ) {
                    $updateQuery = "UPDATE {$sessionRole}s SET fname='{$fname}', lname='{$lname}', email='{$email}', phone='{$phone}', password='{$newPassword}', avatar='{$avatarName}' WHERE id='{$sessionId}'";
